...
if FRAMES_STACKED > 1:
    data = np.hstack(
        [
            train_df.values[i : -FRAMES_STACKED + i - 1, :]
            for i in range(FRAMES_STACKED, 0, -1)
        ]
    )
else:
...
with tf.device(USE_DEVICE):
    ...
    for i in range(NUM_WORKERS):
        ...
        workers.append(
            Worker(env, i, s_size, a_size, trainer, MODEL_DIR, global_episodes)
        )
...
with tf.Session(
    ...
    for worker in workers:
        ...
        def worker_work():
            return worker.work(
                max_episode_len, GAMMA, sess, coord, saver, FRAMES_STACKED
            )
