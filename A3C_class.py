        ...
        with tf.variable_scope(scope):
            ...
            if EXTRA_DENSE:
                hidden = slim.fully_connected(
                    slim.flatten(self.imageIn), N_HIDDEN, activation_fn=tf.nn.tanh
                )
            else:
            ...
            if COOL_A:
                a_in = slim.fully_connected(
                    slim.flatten(rnn_out), 32, activation_fn=tf.nn.tanh
                )
            else:
                a_in = rnn_out
            ...
            if scope != "global":
                ...
                self.loss = (
                    0.5 * self.value_loss + self.policy_loss - self.entropy * 0.01
                )
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                while d == False:
                    ...
                    action_buffer[:] = np.concatenate(
                        (action_buffer[1:], [self.actions[a]])
                    )
                ...
                if episode_count % 50 == 0 and self.name == "worker_0":
                    saver.save(
                        sess, self.model_path + "/model-" + str(episode_count) + ".cptk"
                    )
                    print("Saved Model")
                ...
                summary.value.add(
                    tag="env/shares", simple_value=float(self.env.n_shares)
                )
                summary.value.add(tag="Perf/Act", simple_value=float(a))
                summary.value.add(
                    tag="Perf/Episode_reward", simple_value=float(episode_reward)
                )
